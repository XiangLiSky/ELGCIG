...
for dataset_name in datasets:
    ...
    for size in data_sizes:
        ...
        for params in grid:
            ...
            # 保存原始全连接层的参数
            original_fc_weight = model.classifier[-1].weight.clone().detach()
            original_fc_bias = model.classifier[-1].bias.clone().detach()
            ...
            model.classifier[-1] = torch.nn.Linear(num_ftrs, 1000)
            ...
            for epoch in range(num_epochs):
                ...
                if val_loss < best_loss - min_delta:
                    best_loss = val_loss
                    counter = 0
                    torch.save(model.state_dict(), '../output_enhance_v_l/best_model.pth')
                else:
                    counter += 1
                    if counter >= patience:
                        print(f'Early stopping after {epoch + 1} epochs.')
                        model.load_state_dict(torch.load('../output_enhance_v_l/best_model.pth'))
                        break
            ...
            with torch.no_grad():
                model.classifier[-1].weight.data = alpha * model.classifier[-1].weight.data + (1 - alpha) * original_fc_weight.to(device)
                model.classifier[-1].bias.data = alpha * model.classifier[-1].bias.data + (1 - alpha) * original_fc_bias.to(device)

            # 保存融合后的模型
            torch.save(model.state_dict(), '../output_enhance_v_l/vgg16_retrained_fused.pth')
...
results_df.to_csv('../output_enhance_v_l/enhance_results.csv', index=False)

# 计算每个类别的平均结果
average_results_df = results_df.groupby(['dataset_size', 'alpha']).mean().reset_index()
average_results_df.to_csv('../output_enhance_v_l/enhance_results.csv', index=False)
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_v_l/enhance_alpha_result.jpg')
plt.show()
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_v_l/enhance_number_result.jpg')
plt.show()

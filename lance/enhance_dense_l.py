...
for dataset_name in datasets:
    ...
    # 查找对应的验证集路径
    val_folder_path = f"../datasets/hardImageNet/val/{dataset_name}"
    ...
    for size in data_sizes:
        ...
        for params in grid:
            ...
            # 保存原始全连接层的参数
            original_classifier_weight = model.classifier.weight.clone().detach()
            original_classifier_bias = model.classifier.bias.clone().detach()
            ...
            model.classifier = torch.nn.Linear(num_ftrs, 1000)
            model = model.to(device)

            # 替换最后一层（classifier层）为新的全连接层，新层的参数默认是可训练的
            model.classifier = torch.nn.Linear(num_ftrs, 1000)
            ...
            for epoch in range(num_epochs):
                ...
                if val_loss < best_loss - min_delta:
                    best_loss = val_loss
                    counter = 0
                    torch.save(model.state_dict(), '../output_enhance_d_l/best_model.pth')
                else:
                    counter += 1
                    if counter >= patience:
                        print(f'Early stopping after {epoch + 1} epochs.')
                        model.load_state_dict(torch.load('../output_enhance_d_l/best_model.pth'))
                        break
            ...
            with torch.no_grad():
                model.classifier.weight.data = alpha * model.classifier.weight.data + (1 - alpha) * original_classifier_weight.to(device)
                model.classifier.bias.data = alpha * model.classifier.bias.data + (1 - alpha) * original_classifier_bias.to(device)

            # 保存融合后的模型
            torch.save(model.state_dict(), '../output_enhance_d_l/densenet121_retrained_fused.pth')
            ...
            def calculate_accuracy(trial_runs=10):
                accuracies = []
                image_paths = os.listdir(val_folder_path)
                image_count = 47
...
results_df.to_csv('../output_enhance_d_l/enhance_results.csv', index=False)

# 计算每个类别的平均结果
average_results_df = results_df.groupby(['dataset_size', 'alpha']).mean().reset_index()
average_results_df.to_csv('../output_enhance_d_l/enhance_results.csv', index=False)
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_d_l/enhance_alpha_result.jpg')
plt.show()
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_d_l/enhance_number_result.jpg')
plt.show()

        ...
        for params in grid:
            ...
            for epoch in range(num_epochs):
                ...
                if val_loss < best_loss - min_delta:
                    best_loss = val_loss
                    counter = 0
                    torch.save(model.state_dict(), '../output_enhance_d/best_model.pth')
                else:
                    counter += 1
                    if counter >= patience:
                        print(f'Early stopping after {epoch + 1} epochs.')
                        model.load_state_dict(torch.load('../output_enhance_d/best_model.pth'))
                        break
            ...
            # 保存融合后的模型
            torch.save(model.state_dict(), '../output_enhance_d/densenet121_retrained_fused.pth')
...
results_df.to_csv('../output_enhance_d/enhance_results.csv', index=False)

# 计算每个类别的平均结果
average_results_df = results_df.groupby(['dataset_size', 'alpha']).mean().reset_index()
average_results_df.to_csv('../output_enhance_d/enhance_results.csv', index=False)
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_d/enhance_alpha_result.jpg')
plt.show()
...
# 保存图表为JPG文件
plt.savefig('../output_enhance_d/enhance_number_result.jpg')
plt.show()
